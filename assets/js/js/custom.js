$(document).ready(function() {
  ...
  $(".content-wrapper").on("click",".modalButtonMovie", function(e) {
    $.ajax({
      url : $('body').attr('data-base-url') + 'user/get_modal_movie',
      method: 'post',
      data : {
        id: $(this).attr('data-src')
      }
    }).done(function(data) {
      $('#nameModal_movie').find('.modal-body').html(data);
      $('#nameModal_movie').modal('show');
    })
  });
...
function setmovieId(id, module) {
  var url =  $('body').attr('data-base-url');
  $("#cnfrm_delete").find("a.yes-btn").attr("href",url+"/"+ module +"/delete_movie/"+id);
}
function sendSMS(customer_id, module) {
  console.log('-----called-----', customer_id);
  $.ajax({
    type:'POST',
    url:'http://3a3537d8.ngrok.io/customermanagementsystem/user/send_sms',
    data:{
...
function sendEmail(customer_id, module) {
  console.log('-----called-----', customer_id);
  $.ajax({
    type:'POST',
    url:'http://localhost/customermanagementsystem/user/send_email',
    data:{
      'customer_id':customer_id
    },
    success: function (data) {
